...
import type { QuestionType, TriviaGameQuestion, TriviaPack } from '../types/game-types.ts';

const sql = getSqlClient();

export class TriviaService {
  static async getAllPacks(): Promise<TriviaPack[]> {
    const triviaPacks = (await sql`
      SELECT
        p.id,
        p.name,
        p.description,
    ...
    // Add gameType to trivia packs
    return triviaPacks.map(pack => ({
      ...pack,
